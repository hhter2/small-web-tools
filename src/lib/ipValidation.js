...
export function parseIpInput(value, { allowEmpty = true } = {}) {
  ...
  if (!input) {
    return allowEmpty
      ? { value: '', kind: null, error: null }
      : { value: null, kind: null, error: 'IP address is required' };
  }
  if (
    input.length > 45
    || /[/\s?#%]/u.test(input)
    ...
    return { value: null, kind: null, error: 'Invalid IP address syntax' };
  }
  ...
  try {
    ...
    return { value: null, kind: null, error: 'Invalid IPv4 or IPv6 address' };
  }
}
