...
- **New Security Group**: Created a new category named `Security` (represented by a shield icon) to group tools related to security and code generation. Reclassified both the `QR & Barcode Generator` and the `Secure Password Generator` tools under this new category, making them appear under a separate group in the desktop header navigation dropdown menu, the home grid, and the footer links.
